...
RegisterNetEvent('W3X_towing:openTowingMenu')
AddEventHandler('W3X_towing:openTowingMenu', function()
    OpenTowingMenu()
end)
...
function DetachRope()
    if entity1 and entity2 then
        CloseTowingMenu()
        TriggerServerEvent('W3X_towing:stopTow')
...
function AttemptAttachRope()
    if entity1 and entity2 then
        ...
        TriggerServerEvent('W3X_towing:tow', veh1, veh2)
        CloseTowingMenu()
...
RegisterNetEvent('W3X_towing:removeRope')
AddEventHandler('W3X_towing:removeRope', function(id, veh1_, veh2_)
    for k, rope in pairs(ropes) do
        if rope.id == id then
...
RegisterNetEvent('W3X_towing:makeRope')
AddEventHandler('W3X_towing:makeRope', function(veh1_, veh2_, id, owner)
    for k, rope in pairs(ropes) do
        if rope.id == id then
            DeleteRope(rope.rope)
            ropes[k] = nil
